...
# Modified from transformers.models.mistral.modeling_mistral.MistralMLP with Mistral->Qwen2 and ExpertLinear
class Qwen2MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.gate_proj = ExpertLinear(nn.Linear(self.hidden_size, self.intermediate_size, bias=False))
        self.up_proj = ExpertLinear(nn.Linear(self.hidden_size, self.intermediate_size, bias=False))
        self.down_proj = ExpertLinear(nn.Linear(self.intermediate_size, self.hidden_size, bias=False))
